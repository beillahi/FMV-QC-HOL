...
(*****************************************************************************)
(*We will prove each property of the  above definition in the up coming      *)
...
let is_almost_zero = new_definition
 `is_almost_zero f = !a b. (?k. real_negligible k /\ !x. x IN real_interval[a,b]
  DIFF k ==> f x = Cx(&0))`;;
...
let RINPROD_ZERO_EQ = prove(
 `!f. f IN sq_integrable ==> (r_inprod f f = Cx (&0) <=> is_almost_zero f)`,
